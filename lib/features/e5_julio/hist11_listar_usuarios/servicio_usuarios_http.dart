...
abstract interface class ApiUsuarios {
...
class ServicioUsuariosHttp implements ApiUsuarios {
  ServicioUsuariosHttp({http.Client? cliente})
  ...
  Future<List<dynamic>> obtenerUsuarios() async {
    final respuesta = await _cliente.get(ApiConfig.uri('/users'));
    if (respuesta.statusCode != 200) {
      throw Exception('HTTP ${respuesta.statusCode}');
    }

    final json = jsonDecode(respuesta.body);
    if (json is! List<dynamic>) throw Exception('Lista de usuarios inválida');
    return json;
  }
}
